...
def UPDATESTUDENT(request):
    if request.method == 'POST':
        ...
        student.save()
        messages.success(request, 'Malumotlar muvaffaqqiyatli yangilandi')
...
def DELETECOURSE(request,id):
    course = Course.objects.get(id = id)
    course.delete()
    messages.success(request,'Kurs muvaffaqqiyatli o`chirildi')
    return  redirect('view_course')
...
def DELETESESSION(request,id):
    session = Session_Time.objects.get(id = id)
    session.delete()
    messages.success(request,'Vaqt muvaffaqqiyatli o`chirildi')
    return  redirect('view_session')